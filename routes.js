...
var did_i_tong_it = function(items, tongs) {
	var tonged_items_ids = _.map(tongs, function (tong) {return tong.item.toHexString() });
	_.each(items, function(item) {
		if (tonged_items_ids.indexOf(item._id.toHexString()) !== -1) {
			item.tonged = true;
		}
	});
};
...
var did_i_follow = function(users, followers) {
	var followed_users_id = _.map(followers, function (item) { return item.target.toHexString() });
	_.each(users, function (user){
		if (followed_users_id.indexOf(user._id.toHexString()) !== -1) {
			user.followed = true;
		}
	});
};
...
router.use(function (req, res, next) {
	if (!req.isAuthenticated()) {
		...
		User.findOne({username: req.user.username}).lean().exec(function (err, user) {
			...
			req.user.APP_ID = FeedManager.settins.apiAppId;
			req.user.APP_KEY = FeedManager.settigns.apiKey;
...
router.get('/', function (req, res, next) {
	Item.find({}).populate('user').lean().exec(function (err, popular) {
		...
		if (req.isAuthenticated()) {
			Tong.find({user: req.user.id}).lean().exec(function (err, tonged_items) {
				did_i_tong_it(popular, tonged_items);
...
router.get('/flat', ensureAuthenticated, function (req, res, next) {
	var flatFeed = FeedManager.getNewsFeed(req.user.id)['flat'];

	flatFeed.get({})
		.then(enrichActivities)
		.then(function (enrichActivities) {
...
router.get('/aggregated_feed', ensureAuthenticated, function (req, res, next) {
	var aggregatedFeed = FeedManager.getNewsFeed(req.user.id)['aggregated'];

	aggregatedFeed.get({})
		.then(enrichAggregatedActivities)
		.then(function(enrichAggregatedActivities) {
...
router.get('/notification_feed/', ensureAuthenticated, function (req, res, next) {
	...
	notificationFeed.get({mark_read:true, mark_seen: true})
		...
		.then(function (enrichActivities) {
			res.render('notification_follow', {lastFollower: enrichActivities[0], count: enrichActivities.length, layout: false});
		})
		.catch(next);
});
...
router.get('/people', ensureAuthenticated, function (req, res) {
	User.find({}).lean().exec(function (err, people) {
		Follow.find({user: req.user.id}).exec(function(err, follows) {
			...
			return res.render('people', {location: 'people', user: req.user, people: people, path: req.url, shows_feed: false});
		});
	})
});
...
router.get('/profile/:user', ensureAuthenticated, function (req, res, next) {
	User.findOne({username: req.params.user}, function (err, foundUser){
		...
		var flatFeed = FeedManager.getNewsFeed(foundUser._id)['flat'];
...
router.post('/tong', ensureAuthenticated, function (req, res, next) {
	Item.findOne({_id: req.body.item}, function (err, item) {
		console.log('item', item);
		if (item) {
			var tongData = {user: req.user.id, item: item};
			var tong = new Tong(tongData);
			tong.save(function (err) {
				if (err) next(err);
				res.set('Content-Type', 'application/json');
				return res.send({'tong': {'id': req.body.item}});
			});
		} else {
			res.status(404).send('Not found');
		}
...
router.delete('/tong', ensureAuthenticated, function (req, res) {
	var user = req.user;
	var tongData = {user: req.user.id, item: req.body.item};

	Tong.findOne(tongData, function (err, foundTong) {
		if (foundTong) {
			foundTong.remove();
		}
	});
	...
	res.set('Content-Type', 'application/json');
	return res.send({'pin': req.body.item});
});
